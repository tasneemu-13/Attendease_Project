...
public class ExportAttendanceServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String teacherName = "N/A"; // You may want to fetch this dynamically

        try {
            connection = DatabaseConnection.getConnection();

            // SQL query to get the total lectures and attended lectures per student.
            // A subquery is used to get the total count of classes for the course.
            // The main query then counts attended classes and joins with the student table.
            String query = "SELECT " +
                    "    s.enrollment_id, " +
                    "    s.stu_name, " +
            ...
            try (OutputStream outputStream = response.getOutputStream()) {
                // Call the NEW utility method to handle the summary format
                ExcelUtil.exportAttendanceSummaryToExcel(outputStream, courseCode, courseName, teacherName, studentAttendanceSummary);
            }
