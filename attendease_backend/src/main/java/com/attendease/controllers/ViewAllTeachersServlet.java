...
public class ViewAllTeachersServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            List<Map<String, String>> students = adminServices.viewAllTeachers();

            if (students.isEmpty()) {
                response.setStatus(HttpServletResponse.SC_NO_CONTENT);
                out.print(gson.toJson(Map.of("message", "No teachers available")));
            } else {
                response.setStatus(HttpServletResponse.SC_OK);
                out.print(gson.toJson(students));
            }
        } catch (Exception e) {
            System.err.println("Error in ViewAllTeachersServlet: " + e.getMessage());
            e.printStackTrace();
